...
export default function AddressClientPage() {
  ...
  const [erc20Txs, setErc20Txs] = useState<any[]>([])
  ...
  const [activeTab, setActiveTab] = useState<'transactions' | 'erc20' | 'tokens'>('transactions')

  const [storageCount, setStorageCount] = useState<number>(0)
  const [miningReward, setMiningReward] = useState<string>('0')

  const short = (str?: string) => str && str.length >= 10 ? str.slice(0, 6) + '...' + str.slice(-4) : str || '-'

  useEffect(() => {
    async function load() {
      try {
        const [detail, heldTokens, nativeTx, erc20Tx, storageTx, miner] = await Promise.all([
        ...
        setNativeTxs(nativeTx.list)
        setTxTotal(nativeTx.total)
        setErc20Txs(erc20Tx.list)
        setErc20Total(erc20Tx.total)
    ...
    <main className="p-6 max-w-7xl mx-auto text-white">
      ...
      <div className="responsive-grid mb-6">
        <div className="card flex items-center justify-between">
          <div>
            ...
            <p className="text-2xl font-semibold text-white">
              {addressDetail ? (Number(addressDetail.balance) / 1e18).toFixed(3) : '...'} 0G
            </p>
          </div>
          <FontAwesomeIcon icon={faWallet} className="text-3xl text-yellow-400" />
        </div>
        ...
        <div className="card flex items-center justify-between">
          ...
          <FontAwesomeIcon icon={faCoins} className="text-3xl text-cyan-400" />
        </div>

        <div className="card flex items-center justify-between">
          <div>
            <p className="text-sm text-gray-400">Nonce</p>
            <p className="text-2xl font-semibold text-white">{addressDetail?.nonce ?? '...'}</p>
          </div>
          <FontAwesomeIcon icon={faList} className="text-3xl text-pink-400" />
        </div>

        <div className="card flex items-center justify-between">
          <div>
            <p className="text-sm text-gray-400">Storage Txns</p>
            <p className="text-2xl font-semibold text-white">{storageCount}</p>
          </div>
          <FontAwesomeIcon icon={faServer} className="text-3xl text-purple-400" />
        </div>

        <div className="card flex items-center justify-between">
          <div>
            <p className="text-sm text-gray-400">Mining Reward</p>
            <p className="text-2xl font-semibold text-white">
              {(Number(miningReward) / 1e18).toFixed(4)} 0G
            </p>
          </div>
          <FontAwesomeIcon icon={faCube} className="text-3xl text-green-400" />
        </div>
      </div>
      ...
      {activeTab === 'transactions' && (
        <div className="card overflow-x-auto">
            ...
            <tbody>
              {nativeTxs.map((tx, i) => (
                <tr key={`${tx.hash}-${i}`} className="border-t border-[#30363d] hover:bg-[#0d1117]">
                  ...
                  <td className="px-4 py-2">{tx.epochNumber}</td>
                  <td className="px-4 py-2">{short(tx.from)}</td>
                  <td className="px-4 py-2">{short(tx.to)}</td>
                  ...
                  <td className="px-4 py-2 text-gray-400">{formatAge(Math.floor(Date.now() / 1000 - tx.timestamp))} ago</td>
                </tr>
              ))}
            </tbody>
          </table>
          ...
          <div className="flex justify-between items-center mt-4">
            <button className="text-sm text-white px-3 py-1 rounded border border-[#30363d] hover:bg-[#1f2937]" disabled={txPage === 0} onClick={() => setTxPage(p => Math.max(p - 1, 0))}>
              Previous
            </button>
            <span className="text-sm text-gray-400">Page {txPage + 1} of {Math.ceil(txTotal / 10)}</span>
            <button className="text-sm text-white px-3 py-1 rounded border border-[#30363d] hover:bg-[#1f2937]" disabled={(txPage + 1) * 10 >= txTotal} onClick={() => setTxPage(p => p + 1)}>
              Next
            </button>
          </div>
        </div>
      ...
      {activeTab === 'erc20' && (
        <div className="card overflow-x-auto">
              ...
              {erc20Txs.map((tx, i) => {
                ...
                return (
                  <tr key={`${tx.hash}-${i}`} className="border-t border-[#30363d] hover:bg-[#0d1117]">
                    ...
                    <td className="px-4 py-2">{tx.symbol} ({tx.name})</td>
                    <td className="px-4 py-2 text-gray-400">{formatAge(Math.floor(Date.now() / 1000 - tx.timestamp))} ago</td>
                  </tr>
                )
              })}
          ...
          <div className="flex justify-between items-center mt-4">
            <button className="text-sm text-white px-3 py-1 rounded border border-[#30363d] hover:bg-[#1f2937]" disabled={erc20Page === 0} onClick={() => setErc20Page(p => Math.max(p - 1, 0))}>
              Previous
            </button>
            <span className="text-sm text-gray-400">Page {erc20Page + 1} of {Math.ceil(erc20Total / 10)}</span>
            <button className="text-sm text-white px-3 py-1 rounded border border-[#30363d] hover:bg-[#1f2937]" disabled={(erc20Page + 1) * 10 >= erc20Total} onClick={() => setErc20Page(p => p + 1)}>
              Next
            </button>
          </div>
        </div>
